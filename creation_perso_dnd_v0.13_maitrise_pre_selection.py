...
def update_comboboxes():
    ...
    for combobox, label_text in comboboxes:
        ...
        if "compétences" in label_text.lower():
            values = competences_disponibles
        elif "langues" in label_text.lower():
            values = langues_disponibles
        elif "outils" in label_text.lower():
            values = outils_disponibles
        combobox['values'] = [v for v in values if v not in selected_values or v == current_value]
        if current_value not in combobox['values']:
            combobox.set('')
        combobox.config(postcommand=lambda cb=combobox, opts=values: update_combobox_options(cb, opts))



def update_combobox_options(combobox, values):
    current_value = combobox.get()
    new_values = [v for v in values if v not in selected_values or v == current_value]
    menu = combobox.nametowidget(combobox.menuname)
    menu.delete(0, 'end')
    for value in new_values:
        menu.add_command(label=value, command=tk._setit(combobox, value))
        if value in selected_values and value != current_value:
            menu.entryconfig(value, state='disabled')
...
def update_maitrise():
    ...
    def creer_menu_deroulant(options, label_text, parent_frame, nombre=None):
        ...
        for _ in range(nombre):
            if len(options) == 1:
                ...
                combobox.bind("<<ComboboxSelected>>", lambda event: [update_comboboxes(), update_selected_values()])
    ...
    if outils_historiques:
        ...
        combobox_outils_historique.bind("<<ComboboxSelected>>", lambda event: [update_comboboxes(), update_selected_values()])
    ...
    if choix_outils_historiques:
        ...
        combobox_choix_outils_historique.bind("<<ComboboxSelected>>", lambda event: [update_comboboxes(), update_selected_values()])
        comboboxes.append((combobox_choix_outils_historique, "Outils de l'historique"))
    ...
    if race_selectionnee:
        ...
        for _ in range(races[race_selectionnee].get('nombre_de_langue', 0)):
            ...
            combobox_langues_supplementaires_race.bind("<<ComboboxSelected>>", lambda event: [update_comboboxes(), update_selected_values()])
...
def update_selected_values():
    ...
    selected_values = [combobox.get() for combobox, label_text in comboboxes if combobox.get()]
    update_comboboxes()
