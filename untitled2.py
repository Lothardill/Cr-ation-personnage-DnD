...
class Personnage:
    def __init__(self, classes, races, historiques, parent):
        ...
        # Créer le combobox pour la race et l'associer à l'instance
        tk.Label(parent, text="Sélectionnez une race :").grid(row=0, column=0)
        self.race_combobox = ttk.Combobox(parent, values=list(self.races.keys()), state="readonly")
        self.race_combobox.grid(row=1, column=0)
        self.race_combobox.bind("<<ComboboxSelected>>", self.update_display)
        
    def update_display(self, event=None):
        if self.race_combobox.get():  # Utilise la combobox de l'instance
            selected_race = self.race_combobox.get()
            bonus = self.races[selected_race]["bonus"]
            print(f"Bonus de la race sélectionnée : {bonus}")
        else:
            print("Aucune race sélectionnée.")
    ...
    def update_comboboxes(self):
        ...
        # Extraire les compétences et langues disponibles en fonction de la classe, de la race, et de l'historique sélectionnés
        ...
        for combobox, label_text in self.comboboxes:
            values = []  # Initialiser une liste vide pour stocker les valeurs possibles pour chaque combobox
        
            # Mettre à jour les comboboxes en fonction de leur étiquette
            if "compétences de la classe" in label_text.lower():
                values = list(competences_disponibles_classe)  # Récupérer uniquement les compétences de la classe
                print(f"Compétences de la classe: {values}")
            elif "compétences de la race" in label_text.lower():
                values = self.races[race_selectionnee].competences if race_selectionnee else []
                print(f"Compétences de la race: {values}")
            elif "compétences de l'historique" in label_text.lower():
                values = competences_disponibles_historique
                print(f"Compétences de l'historique: {values}")
            elif "langues de la race" in label_text.lower():
                values = langues_disponibles
                print(f"Langues de la race: {values}")
            elif "langues de l'historique" in label_text.lower():
                values = langues_disponibles
                print(f"Langues de l'historique: {values}")
            elif "outils de la race" in label_text.lower():
                values = self.races[race_selectionnee].outils if race_selectionnee else []
                print(f"Outils de la race: {values}")
            elif "outils de la classe" in label_text.lower():
                values = self.classes[classe_selectionnee].outils if classe_selectionnee else []
                print(f"Outils de la classe: {values}")
            elif "outils de l'historique" in label_text.lower():
                if label_text == "Outils de l'historique":
                    values = self.historiques[historique_selectionne].outils if historique_selectionne else []
            elif label_text == "Choix d'outils de l'historique":
                values = self.historiques[historique_selectionne].choix_outils if historique_selectionne else []
                print(f"Outils de l'historique: {values}")
        
            # Mettre à jour les options de la combobox avec les valeurs trouvées
    ...
    def afficher_competences_race(self, race_selectionnee, maitrise_frame):
        # Récupérer les informations de la race sélectionnée
        race_info = self.races[race_selectionnee]
        competences_race = race_info.competences
        nombre_competences_race = len(competences_race)

        # Si une seule compétence, on l'affiche en texte
        if nombre_competences_race == 1:
            tk.Label(maitrise_frame, text="Compétence de la race :", font=('Arial', 10, 'bold')).grid(row=0, column=0, sticky="w")
            tk.Label(maitrise_frame, text=competences_race[0], font=('Arial', 10)).grid(row=1, column=0, sticky="w")

        # Si plusieurs compétences, on affiche un menu déroulant
        elif nombre_competences_race > 1:
            tk.Label(maitrise_frame, text="Compétences de la race :", font=('Arial', 10, 'bold')).grid(row=0, column=0, sticky="w")
            combobox_competences_race = ttk.Combobox(maitrise_frame, values=competences_race, state="readonly")
            combobox_competences_race.grid(row=1, column=0, padx=5, pady=5)

    def update_maitrise(self):
        
        # Bind les comboboxes pour que la fonction se mette à jour à chaque modification
        race_combobox.bind("<<ComboboxSelected>>", lambda event: self.update_maitrise())
        classe_combobox.bind("<<ComboboxSelected>>", lambda event: self.update_maitrise())
        historique_combobox.bind("<<ComboboxSelected>>", lambda event: self.update_maitrise())
    
        # Récupérer la race, la classe, et l'historique sélectionnés
        classe_selectionnee = classe_combobox.get()
        race_selectionnee = self.race_combobox.get()
        historique_selectionne = historique_combobox.get()

        # Réinitialiser le cadre de maîtrise
        for widget in maitrise_frame.winfo_children():
            widget.destroy()

        self.comboboxes = []

        # Récupérer les compétences et outils disponibles
        ...
        self.afficher_competences_race(race_selectionnee, self.maitrise_frame)

        # --- Gérer les compétences de la classe ---
        if classe_selectionnee:
            competences_classe = self.classes[classe_selectionnee].competences
            nombre_de_competence_classe = self.classes[classe_selectionnee].nombre_de_competence

            # Appel de la fonction creer_menu_deroulant pour les compétences de la classe
            self.creer_menu_deroulant(competences_classe, "Compétences de la classe", self.maitrise_frame, nombre=nombre_de_competence_classe)


        # --- Gérer les compétences de l'historique ---
        if historique_selectionne:
            nombre_competences_historique = self.historiques[historique_selectionne].nombre_de_competence
            competences_historique = self.historiques[historique_selectionne].competences
            self.creer_menu_deroulant(competences_historique, "Compétences de l'historique", maitrise_frame, nombre=nombre_competences_historique)

        # --- Gérer les outils de la race ---
        if race_selectionnee:
            race_info = self.races[race_selectionnee]
            outils_race = race_info.outils

            if outils_race:
                self.creer_menu_deroulant(outils_race, "Outils de la race", maitrise_frame, nombre=len(outils_race))

        # --- Gérer les outils de la classe ---
        if classe_selectionnee:
            outils_classe = self.classes[classe_selectionnee].outils
            self.creer_menu_deroulant(outils_classe, "Outils de la classe", maitrise_frame, nombre=1)

        # --- Gérer les outils de l'historique ---
        if historique_selectionne:
        ...
        # --- Gérer les langues de l'historique ---
        if historique_selectionne:
        ...
        # --- Réinitialiser les comboboxes lorsque les sélections changent ---
        self.update_comboboxes()      
