...
def extraire_competences_classe_selectionnees():
...
def update_options():
    ...
    if classe_selectionnee == 'Roublard':
        ...
        competences_disponibles = [clean_value(cb.get()) for cb, label in comboboxes if "Compétences de la classe" in label or "Compétences de l'historique" in label or "Compétences de la race" in label or "Outils de la classe" in label or "Outils de la race" in label or "Outils de l'historique" in label]
        
        expertise_frame = tk.Frame(options_frame)
        expertise_frame.pack(anchor="w", pady=5)
        competences_classe_selectionnees = extraire_competences_classe_selectionnees()
        for i in range(2):  # Deux menus déroulants pour l'expertise
            expertise_combobox = ttk.Combobox(expertise_frame, values=competences_classe_selectionnees, state="readonly")
