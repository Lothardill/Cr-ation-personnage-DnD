...
def update_display():
    for i, attribut in enumerate(attributs_list):
        ...
        attribut_labels[i].config(text=f"{attribut}: {valeur}")
        attribut_bonus_labels[i].config(text=f"Bonus racial: {bonus}")
        attribut_mod_labels[i].config(text=f"({modificateur:+d})")
...
attribut_mod_labels = []
for i, attribut in enumerate(attributs_list):
    tk.Label(root, text=attribut).grid(row=10+i, column=0)
    attribut_label = tk.Label(root, text=f"{attribut}: {attributs_personnage[attribut]}")
    attribut_label.grid(row=10+i, column=1)
    attribut_labels.append(attribut_label)
    attribut_bonus_label = tk.Label(root, text="")
    attribut_bonus_label.grid(row=10+i, column=2)
    attribut_bonus_labels.append(attribut_bonus_label)
    attribut_mod_label = tk.Label(root, text="")
    attribut_mod_label.grid(row=10+i, column=3)
    attribut_mod_labels.append(attribut_mod_label)
    tk.Button(root, text="+", command=lambda a=attribut: augmenter_attribut(a)).grid(row=10+i, column=4)
    tk.Button(root, text="-", command=lambda a=attribut: diminuer_attribut(a)).grid(row=10+i, column=5)

# Points restants
points_label = tk.Label(root, text=f"Points restants: {points_disponibles}")
points_label.grid(row=16, column=0, columnspan=2, pady=10)

# Bouton de validation
tk.Button(root, text="Valider répartition", command=valider_repartition).grid(row=16, column=2, columnspan=2, pady=10)
