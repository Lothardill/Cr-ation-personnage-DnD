...
# Boutons pour Lignée Draconique et Magie Sauvage (placed side by side)
lignée_button = tk.Button(button_frame, text="Afficher Lignée Draconique", command=lambda: toggle_lignée())
lignée_button.pack(side="left", padx=10)  # Left side with padding
lignée_button.pack_forget()  # Initially hidden

magie_sauvage_button = tk.Button(button_frame, text="Afficher Magie Sauvage", command=lambda: toggle_magie_sauvage())
magie_sauvage_button.pack(side="right", padx=10)  # Right side with padding
magie_sauvage_button.pack_forget()  # Initially hidden


# Contenu caché pour Lignée Draconique and Magie Sauvage
lignée_text = tk.Label(onglet_classe, text="""Lignée draconique:
...
Interaction draconique : Vous pouvez parler, lire et écrire le draconique. De plus, chaque fois que vous faites un jet de Charisme pour interagir avec des dragons, votre bonus de maîtrise est doublé s’il s’applique au jet.

Résistance draconique : La magie qui coule à travers votre corps fait émerger des traits physiques de vos ancêtres dragons. Au niveau 1, votre maximum de points de vie augmente de 1 et également de 1 à chaque fois que vous gagnez un niveau dans cette classe. En outre, des parties de votre peau sont couvertes d’un mince reflet d’écailles de dragon. Lorsque vous ne portez pas d'armure, votre CA est égale à 13 + votre modificateur de Dextérité.
""", wraplength=700, justify="left")
lignée_text.pack_forget()  # Initially hidden

magie_sauvage_text = tk.Label(onglet_classe, text="""Magie Sauvage:
...
def afficher_details_classe():
    ...
    if classe_selectionnee:
        ...
        if classe_selectionnee == "Ensorceleur":
            ...
            button_frame.pack(pady=5)  # Place the frame containing both buttons
            lignée_button.pack(side="left", padx=10)  # Show Lignée Draconique button
            magie_sauvage_button.pack(side="right", padx=10)  # Show Magie Sauvage button
            create_magie_sauvage_table()
        else:
            # Hide buttons and their contents for other classes
            lignée_button.pack_forget()
            magie_sauvage_button.pack_forget()
            lignée_text.pack_forget()
            magie_sauvage_text.pack_forget()
...
# Function to toggle Lignée Draconique visibility
def toggle_lignée():
    if lignée_text.winfo_viewable():
        lignée_text.pack_forget()
        lignée_button.config(text="Afficher Lignée Draconique")
    else:
        lignée_text.pack(anchor="w", pady=5)
        lignée_button.config(text="Masquer Lignée Draconique")

# Function to toggle Magie Sauvage visibility
def toggle_magie_sauvage():
    if magie_sauvage_text.winfo_viewable():
        magie_sauvage_text.pack_forget()
        magie_sauvage_button.config(text="Afficher Magie Sauvage")
        magie_sauvage_table_frame.pack_forget()  # Hide the table
        
        
    else:
        magie_sauvage_text.pack(anchor="w", pady=5)
        magie_sauvage_button.config(text="Masquer Magie Sauvage")
        magie_sauvage_table_frame.pack(pady=10)  # Show the table
        

def create_magie_sauvage_table():
    global magie_sauvage_table_frame

    # Check if the frame already exists to avoid recreating
    if magie_sauvage_table_frame is not None:
        return

    # Create a frame to contain the table
    magie_sauvage_table_frame = tk.Frame(onglet_classe)

    # Create the table with tkinter's Treeview
    colonnes = ("d100", "Effet")
    magie_sauvage_table = ttk.Treeview(magie_sauvage_table_frame, columns=colonnes, show='headings', height=10)
    ...
    magie_sauvage_table.column("Effet", width=600, anchor="w")
    ...
    for row in data:
        magie_sauvage_table.insert('', tk.END, values=row)

    # Pack the table into the frame
    magie_sauvage_table.pack(fill="both", expand=True)
