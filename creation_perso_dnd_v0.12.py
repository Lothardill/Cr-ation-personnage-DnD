...
def update_comboboxes():
    ...
    if race_selectionnee:
        ...
        langues_disponibles -= langues_parlees_race
    ...
    for combobox, label_text in comboboxes:
        if "compétences de la classe" in label_text.lower():
            ...
            historique_selectionne = historique_combobox.get()
            if historique_selectionne:
                values = historiques[historique_selectionne].get('compétences', [])
        ...
        elif "choix d'outils de l'historique" in label_text.lower():
            historique_selectionne = historique_combobox.get()
            if historique_selectionne:
                values = historiques[historique_selectionne].get('choix_outils', [])
        
        update_combobox_options(combobox, values)
...
def update_maitrise():
    ...
    if race_selectionnee:
        ...
        if len(outils_race) == 1:
            outils_race = [format_value(outils_race[0])]
        creer_menu_deroulant(outils_race, "Outils de la race", maitrise_frame, 1)

    # Outils de la classe
    if classe_selectionnee:
        outils_classe = classes[classe_selectionnee].get('outils', [])
        if len(outils_classe) == 1:
            outils_classe = [format_value(outils_classe[0])]
        creer_menu_deroulant(outils_classe, "Outils de la classe", maitrise_frame, 1)
    ...
    if historique_selectionne:
        ...
        if outils_historiques:
            if len(outils_historiques) == 1:
                outils_historiques = [format_value(outils_historiques[0])]
        ...
        if choix_outils_historiques:
            if len(choix_outils_historiques) == 1:
                choix_outils_historiques = [format_value(choix_outils_historiques[0])]
            combobox_choix_outils_historiques = ttk.Combobox(outils_historiques_frame, values=sorted(choix_outils_historiques), state="readonly")
    ...
    if race_selectionnee:
        ...
        for _ in range(races[race_selectionnee].get('nombre_de_langue', 0)):
            combobox_langues_supplementaires_race = ttk.Combobox(langues_race_frame, values=all_langues, state="readonly")
